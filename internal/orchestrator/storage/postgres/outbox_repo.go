...
// Add вставляет команду в outbox таблицу внутри транзакции
func (r *OutboxRepo) Add(ctx context.Context, tx *sqlx.Tx, eventType string, aggregateID string, sagaID uuid.UUID, payload interface{}) error {
	const query = `
		INSERT INTO orchestrator_outbox (event_id, event_type, aggregate_id, saga_id, payload, occurred_at)
		VALUES ($1, $2, $3, $4, $5, $6)
	`
	...
	if err != nil {
		return fmt.Errorf("insert orchestrator outbox: %w", err)
	}
	return nil
}
...
func (r *OutboxRepo) MarkProcessed(ctx context.Context, id int64) error {
	const q = `UPDATE orchestrator_outbox SET processed_at = NOW() WHERE id = $1`
