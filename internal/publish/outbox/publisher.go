...
func NewPublisher(cfg PublisherConfig) (*Publisher, error) {
	...
	return &Publisher{
		...
		logger:     cfg.Logger.With().Str("component", "publish_outbox_publisher").Logger(),
	}, nil
}
...
func (p *Publisher) Start(ctx context.Context) error {
	...
	p.logger.Info().
		...
		Msg("publish outbox publisher started")

	for {
		select {
		case <-ctx.Done():
			p.logger.Info().Msg("publish outbox publisher stopped")
			return ctx.Err()
		case <-ticker.C:
...
func mapEventTypeToTopic(eventType string) string {
	...
	case "PublishSucceeded":
		return "events.publish.succeeded"
	case "PublishFailed":
		return "events.publish.failed"
	default:
		return "events.publish.unknown"
	}
}
