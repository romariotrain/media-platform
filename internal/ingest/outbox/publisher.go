...
// Publisher реализует Outbox паттерн для Ingest сервиса.
// Polling-based: читает pending события из БД и публикует в Kafka.
...
func NewPublisher(cfg PublisherConfig) (*Publisher, error) {
	...
	return &Publisher{
		...
		logger:     cfg.Logger.With().Str("component", "ingest_outbox_publisher").Logger(),
	}, nil
}
...
func (p *Publisher) Start(ctx context.Context) error {
	...
	p.logger.Info().
		...
		Msg("ingest outbox publisher started")

	for {
		select {
		case <-ctx.Done():
			p.logger.Info().Msg("ingest outbox publisher stopped")
			return ctx.Err()
		case <-ticker.C:
...
func mapEventTypeToTopic(eventType string) string {
	switch eventType {
	case "IngestReady":
		return "events.ingest.ready"
	case "IngestUploaded":
		return "events.ingest.uploaded"
	case "IngestFailed":
		return "events.ingest.failed"
	default:
		return "events.ingest.unknown"
	}
}
